...
- 完成交付 tester → 附：分支名、改动摘要、注意事项。**同时通知 main（附交付摘要），让 main 立即转发 用户。**
...
- [ ] 自测通过
- [ ] git add + git commit
- [ ] **git push origin feature/xxx**（tester 需要拉取代码）
- [ ] **已写入 docs/journey.md**：记录阶段6完成时间、代码文件列表
- [ ] sessions_send → tester（附分支名和改动摘要）
...
- 缺陷模板：`projects/ma/tester/bug-report.md` — tester 会按此模板登记 bug
