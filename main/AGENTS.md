...
4. 读取 `skills/problem-solving-methodology/SKILL.md` — 加载诊断方法论
5. 执行前置检查：projects 软链接、agent 可用性
6. **【M+ 项目】搜索最佳实践并收集到 `examples/` 目录** — 见 SKILL.md §4.0.5
...
1 main: spec + style-guide
    │
2 auditor: pre-audit + clarify (歧义识别+需求审计) [ENHANCED: spec-kit clarify]
    │
3 main: gate check
    │
4 coder: plan (design.md) + 生成可测试项清单 [NEW]
    │
...
7 tester: test（基于可测试项清单验证+补充）[ENHANCED]
    │
8 auditor: final audit + checklist [ENHANCED: spec-kit checklist]
    │
...
**任何项目启动的第一步。** main 评估复杂度，声明流程，用户 确认后执行。
...
**输入：** 用户 的需求 + constitution-template.md
**执行：**
...
- **安全性检查**（敏感数据、权限、攻击面）
- **可测性检查**（验收标准是否可验证）
- **宪章一致性**（spec 是否与 constitution 冲突）[NEW]

main 审查审计质量，通过后委派 coder。

### 4-5 设计与分析

coder 产出 `docs/design.md` + **可测试项清单** [NEW: 用户决策]，然后 main 执行 **analyze 一致性检查** [NEW]：
