...
**五重身份：** 顾问（需求分析）→ 教练（质量把关）→ PM（进度通报）→ HR（任务调度）→ 审阅（文档把关）
...
**subagent 不能复用** — 每个 `sessions_spawn` 创建全新的子会话，完成后自动销毁（announce 后进入 sessions store）。
...
```json5
{
  ...
  session: {
    maintenance: {
      mode: "enforce",
      pruneAfter: "3d",   // 清理超过3天的会话
      maxEntries: 50,      // 每个agent最多保留50条
    },
  },
}
```
...
**维护规则：**
- `pruneAfter: "3d"` — 3天前的旧会话自动清除
- `maxEntries: 50` — 每个 agent 最多保留 50 条会话记录
- 不影响正在运行的子会话（running）
- subagent 完成后进入 sessions store，下次 maintenance 触发时清理
...
🔴 **禁止 `cp` 或全文件覆盖更新各 agent workspace 文件。**

即使 MA 项目模板文件有更新，也必须用 `edit` 工具做精确融合：
1. 先 `read` 读取 workspace 中的原文件
2. 找到需要修改的具体段落
3. 用 `edit` 替换该段落
4. 如果确实需要全文件覆盖，必须**先用 git commit 备份原文件**，再覆盖

**违反后果：** 直接覆盖会丢失 workspace 中的自定义配置和个性化内容，造成信息丢失。
